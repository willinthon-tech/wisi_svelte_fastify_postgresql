...
async function testSingle() {
  const endpoint = "/ISAPI/AccessControl/AcsEventSearch?format=json";
  const deviceUrl = `http://${dev2.ip_local}`;
  const fullUrl = deviceUrl + endpoint;
  const bodyObj = {
    AcsEventCond: { searchID: "1", searchResultPosition: 0, maxResults: 10, major: 5, minor: 0 }
  };
  const bodyStr = JSON.stringify(bodyObj);

  console.log(`📡 Testing AcsEventSearch on DS-K1T343MWX (192.168.3.174)...`);
  ...
  try {
    ...
    if (wwwAuthenticate && wwwAuthenticate.includes('Digest')) {
      ...
      console.log(`🎉 SUCCESS for AcsEventSearch! Status: ${res2.status}`);
