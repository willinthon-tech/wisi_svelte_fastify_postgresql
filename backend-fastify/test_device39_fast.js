...
async function testFast(label, bodyObj) {
  ...
  try {
    ...
    if (www && www.includes('Digest')) {
      ...
      console.log(`🎉 RESULT for [${label}]! Status: ${res2.status} in ${Date.now() - start}ms`);
...
async function runAll() {
  await testFast("timeReverseOrder: false", {
    searchID: "1",
  ...
  await testFast("major: 5, minor: 0, picEnable: false, timeReverseOrder: false", {
    searchID: "1",
    searchResultPosition: 0,
    maxResults: 10,
    major: 5,
