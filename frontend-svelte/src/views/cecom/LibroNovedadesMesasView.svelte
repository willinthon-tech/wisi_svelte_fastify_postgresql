<script>
  ...
  $: filteredMesas = (() => {
    ...
    return availableMesas.filter(m => {
      ...
      const targetText = normalizeSearchStr(`${m.nombre || ''} ${m.juego_nombre || ''} ${r?.pitboss || ''} ${r?.croupier_apertura || ''} ${r?.croupier_cierre || ''} ${r?.observacion || ''}`);
  ...
  // Lista de empleados disponibles para sugerencias y autocompletado (filtrados por la sala del libro con fallback)
  $: listaEmpleados = (() => {
    let filtered = allEmpleados.filter(e => {
      ...
      if (assignedSalaUuids && assignedSalaUuids.length > 0 && eSala) {
        return assignedSalaUuids.some(su => String(su) === eSala);
      }
      return !eSala;
    });
  ...
  $: currentQueryCroupier = (activeSug && rowsData[activeSug.mesaId]) 
    ? (rowsData[activeSug.mesaId][activeSug.field] || '').trim() 
    : '';
  ...
  $: currentQueryPitboss = pitbossSugTarget === 'batch'
    ? (batchPitboss || '').trim()
    : (pitbossSugTarget === 'modal' ? (modalPitboss || '').trim() : '');
