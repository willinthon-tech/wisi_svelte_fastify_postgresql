<script context="module">
  ...
  export const persistentLlavesFilters = writable({
    selectedSalas: [],
    searchQuery: ""
  });
</script>
...
<script>
  ...
  let initial = {};
  const unsubInit = persistentLlavesFilters.subscribe((val) => {
    initial = val || {};
  });
  unsubInit();
  ...
  let items = [];
  $: allLlaves = ($masterLlavesStore || []).filter(m => (m.active ?? 1) === 1);
  let totalCount = 0;
  let currentPage = 1;
  let pageSize = 10;
...
<PaginatedDataTable 
  ...
  {createFields}
  bind:searchQuery
  searchPlaceholder="Buscar llaves por nombre, sala o ID..."
  entityType="llave"
  ...
  actions={{ 
    ...
    deleteTitle: 'Desincorporar Llave' 
  }}
