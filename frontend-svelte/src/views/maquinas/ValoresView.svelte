...
<script>
  ...
  async function loadServerData(params = {}) {
    ...
    try {
      ...
      console.error(err);
      triggerToast('Error al cargar valores del servidor', 'error');
    }
  }
  ...
  $: columns = [
    ...
    { key: 'nombre', label: 'Valor / Denominación', bold: true, sortable: true, editable: true }
  ];

  $: createFields = [
    { key: 'nombre', label: 'Valor / Denominación', type: 'text', placeholder: 'Ej. 0.01, 0.05, 1.00, 10.00...', required: true }
  ];

  async function handleCreate(event) {
    const draft = event.detail;
    try {
      await masterValoresActions.add(draft);
  ...
  async function handleSaveInline(event) {
    ...
    try {
      await masterValoresActions.update(id, draft);
