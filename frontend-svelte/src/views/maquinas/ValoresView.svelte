<script context="module">
  ...
  export const persistentValoresFilters = writable({
    searchQuery: ""
  });
</script>
...
<script>
  ...
  import { masterValoresActions, masterValoresStore, loadMasterStoresFromBackend } from '../../controllers/master.store.js';
  import { getLocalItems, saveLocalItems } from '../../services/localDb.service.js';
  import { triggerToast } from '../../controllers/ui.store.js';

  let initial = {};
  const unsubInit = persistentValoresFilters.subscribe((val) => {
    initial = val || {};
  });
  unsubInit();
  ...
  onMount(async () => {
    ...
    try {
      const local = await getLocalItems('valores');
      if (Array.isArray(local) && local.length > 0) {
        items = local;
        totalCount = local.length;
      } else if (Array.isArray(allValores) && allValores.length > 0) {
        items = allValores;
        totalCount = allValores.length;
      }
    } catch (e) {}
  ...
  async function loadServerData(params = {}) {
    ...
    try {
      ...
      const res = await fetch(`/api/master/valores?${q.toString()}`);
      ...
      if (json && json.success) {
        ...
        saveLocalItems('valores', items).catch(() => {});
      }
    } catch (err) {
      console.warn('Fallback local IndexedDB para valores:', err);
      const local = await getLocalItems('valores');
      const source = (Array.isArray(local) && local.length > 0) ? local : ($masterValoresStore || []);
  ...
  $: columns = [
    ...
    { key: 'nombre', label: 'Denominación / Valor', bold: true, sortable: true, editable: true }
  ];

  $: createFields = [
    { key: 'nombre', label: 'Nombre del Valor', type: 'text', placeholder: 'Ej. Alto, Medio, Bajo, Especial...', required: true }
  ];

  async function handleCreate(event) {
    const draft = event.detail;
    try {
      const created = await masterValoresActions.add(draft);
      triggerToast('Valor creado exitosamente', 'success');
      if (created) {
      ...
      triggerToast(`Error al crear valor: ${err.message}`, 'error');
    }
  }
  ...
  async function handleSaveInline(event) {
    ...
    const targetUuid = id;
    try {
      await masterValoresActions.update(targetUuid, draft);
      triggerToast('Valor actualizado exitosamente', 'success');
      items = items.map(x => (String(x.uuid || x.id) === String(targetUuid)) ? { ...x, ...draft } : x);
      loadServerData().catch(() => {});
    } catch (err) {
      triggerToast(`Error al actualizar valor: ${err.message}`, 'error');
    }
  }
  ...
  async function handleDelete(event) {
    ...
    try {
      const res = await masterValoresActions.delete(targetUuid);
      if (res && res.blocked) {
        if (onResult) {
          onResult(res);
        } else {
        ...
        triggerToast(`Valor eliminado exitosamente`, 'success');
      ...
      triggerToast(`Error al eliminar valor: ${err.message}`, 'error');
    }
  }
  ...
  async function handleBatchDelete(event) {
    ...
    for (const id of ids) {
      try {
        const res = await masterValoresActions.delete(id);
        if (res && res.blocked) {
          blocked.push({
            id,
...
<PaginatedDataTable 
  ...
  {createFields}
  bind:searchQuery
  searchPlaceholder="Buscar valores por denominación o ID..."
  entityType="valor"
