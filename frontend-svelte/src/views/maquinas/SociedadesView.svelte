...
<script>
  ...
  async function loadServerData(params = {}) {
    ...
    try {
      ...
      console.error(err);
      triggerToast('Error al cargar sociedades del servidor', 'error');
    }
  }
  ...
  $: createFields = [
    { key: 'nombre', label: 'Nombre de la Sociedad', type: 'text', placeholder: 'Ej. Inversiones Casino C.A., Operadora Wisi...', required: true }
  ];

  async function handleCreate(event) {
    const draft = event.detail;
    try {
      await masterSociedadesActions.add(draft);
  ...
  async function handleSaveInline(event) {
    ...
    try {
      await masterSociedadesActions.update(id, draft);
