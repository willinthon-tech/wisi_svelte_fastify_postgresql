<script context="module">
  ...
  export const persistentSociedadesFilters = writable({
    searchQuery: ""
  });
</script>
...
<script>
  ...
  import { masterSociedadesActions, masterSociedadesStore, loadMasterStoresFromBackend } from '../../controllers/master.store.js';
  import { getLocalItems, saveLocalItems } from '../../services/localDb.service.js';
  import { triggerToast } from '../../controllers/ui.store.js';

  let initial = {};
  const unsubInit = persistentSociedadesFilters.subscribe((val) => {
    initial = val || {};
  });
  unsubInit();
  ...
  $: {
    persistentSociedadesFilters.set({
      searchQuery
    });
  }
  ...
  let items = [];
  $: allSociedades = $masterSociedadesStore || [];
  let totalCount = 0;
  let currentPage = 1;
  let pageSize = 10;
  ...
  async function loadServerData(params = {}) {
    ...
    try {
      ...
      const res = await fetch(`/api/master/sociedades?${q.toString()}`);
  ...
  $: columns = [
    ...
    { key: 'nombre', label: 'Nombre de la Sociedad', bold: true, sortable: true, editable: true }
  ];

  $: createFields = [
    { key: 'nombre', label: 'Nombre de la Sociedad', type: 'text', placeholder: 'Ej. Grupo A, Operadora Norte...', required: true }
  ];

  async function handleCreate(event) {
    const draft = event.detail;
    try {
      const created = await masterSociedadesActions.add(draft);
      triggerToast('Sociedad creada exitosamente', 'success');
      if (created) {
      ...
      triggerToast(`Error al crear sociedad: ${err.message}`, 'error');
    }
  }
  ...
  async function handleSaveInline(event) {
    ...
    const targetUuid = id;
    try {
      await masterSociedadesActions.update(targetUuid, draft);
      triggerToast('Sociedad actualizada exitosamente', 'success');
      items = items.map(x => (String(x.uuid || x.id) === String(targetUuid)) ? { ...x, ...draft } : x);
      loadServerData().catch(() => {});
    } catch (err) {
      triggerToast(`Error al actualizar sociedad: ${err.message}`, 'error');
    }
  }
  ...
  async function handleDelete(event) {
    ...
    try {
      const res = await masterSociedadesActions.delete(targetUuid);
      if (res && res.blocked) {
        if (onResult) {
          onResult(res);
        } else {
        ...
        triggerToast(`Sociedad eliminada exitosamente`, 'success');
      ...
      triggerToast(`Error al eliminar sociedad: ${err.message}`, 'error');
    }
  }
  ...
  async function handleBatchDelete(event) {
    ...
    for (const id of ids) {
      try {
        const res = await masterSociedadesActions.delete(id);
        if (res && res.blocked) {
          blocked.push({
            id,
...
<PaginatedDataTable 
  ...
  {createFields}
  bind:searchQuery
  searchPlaceholder="Buscar sociedades por nombre o ID..."
  entityType="sociedad"
