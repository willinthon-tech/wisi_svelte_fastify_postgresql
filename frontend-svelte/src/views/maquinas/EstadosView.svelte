...
<script>
  ...
  async function loadServerData(params = {}) {
    ...
    try {
      ...
      console.error(err);
      triggerToast('Error al cargar estados del servidor', 'error');
    }
  }
  ...
  $: createFields = [
    { key: 'nombre', label: 'Nombre del Estado', type: 'text', placeholder: 'Ej. Activa, En Mantenimiento, Apagada...', required: true }
  ];

  async function handleCreate(event) {
    const draft = event.detail;
    try {
      await masterEstadosActions.add(draft);
  ...
  async function handleSaveInline(event) {
    ...
    try {
      await masterEstadosActions.update(id, draft);
