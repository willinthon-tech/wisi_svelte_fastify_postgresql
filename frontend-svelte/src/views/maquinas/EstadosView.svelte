<script context="module">
  ...
  export const persistentEstadosFilters = writable({
    searchQuery: ""
  });
</script>
...
<script>
  ...
  import { masterEstadosActions, masterEstadosStore, loadMasterStoresFromBackend } from '../../controllers/master.store.js';
  import { getLocalItems, saveLocalItems } from '../../services/localDb.service.js';
  import { triggerToast } from '../../controllers/ui.store.js';

  let initial = {};
  const unsubInit = persistentEstadosFilters.subscribe((val) => {
    initial = val || {};
  });
  unsubInit();
  ...
  onMount(async () => {
    ...
    try {
      const local = await getLocalItems('estados');
      if (Array.isArray(local) && local.length > 0) {
        items = local;
        totalCount = local.length;
      } else if (Array.isArray(allEstados) && allEstados.length > 0) {
        items = allEstados;
        totalCount = allEstados.length;
      }
    } catch (e) {}
  ...
  async function loadServerData(params = {}) {
    ...
    try {
      ...
      const res = await fetch(`/api/master/estados?${q.toString()}`);
      ...
      if (json && json.success) {
        ...
        saveLocalItems('estados', items).catch(() => {});
      }
    } catch (err) {
      console.warn('Fallback local IndexedDB para estados:', err);
      const local = await getLocalItems('estados');
      const source = (Array.isArray(local) && local.length > 0) ? local : ($masterEstadosStore || []);
  ...
  $: columns = [
    ...
    { key: 'nombre', label: 'Nombre del Estado', bold: true, sortable: true, editable: true }
  ];

  $: createFields = [
    { key: 'nombre', label: 'Nombre del Estado', type: 'text', placeholder: 'Ej. Operativa, En Taller, Desactivada...', required: true }
  ];

  async function handleCreate(event) {
    const draft = event.detail;
    try {
      const created = await masterEstadosActions.add(draft);
      triggerToast('Estado creado exitosamente', 'success');
      if (created) {
      ...
      triggerToast(`Error al crear estado: ${err.message}`, 'error');
    }
  }
  ...
  async function handleSaveInline(event) {
    ...
    const targetUuid = id;
    try {
      await masterEstadosActions.update(targetUuid, draft);
      triggerToast('Estado actualizado exitosamente', 'success');
      items = items.map(x => (String(x.uuid || x.id) === String(targetUuid)) ? { ...x, ...draft } : x);
      loadServerData().catch(() => {});
    } catch (err) {
      triggerToast(`Error al actualizar estado: ${err.message}`, 'error');
    }
  }
  ...
  async function handleDelete(event) {
    ...
    try {
      const res = await masterEstadosActions.delete(targetUuid);
      if (res && res.blocked) {
        if (onResult) {
          onResult(res);
        } else {
        ...
        triggerToast(`Estado eliminado exitosamente`, 'success');
      ...
      triggerToast(`Error al eliminar estado: ${err.message}`, 'error');
    }
  }
  ...
  async function handleBatchDelete(event) {
    ...
    for (const id of ids) {
      try {
        const res = await masterEstadosActions.delete(id);
        if (res && res.blocked) {
          blocked.push({
            id,
...
<PaginatedDataTable 
  ...
  {createFields}
  bind:searchQuery
  searchPlaceholder="Buscar estados por nombre o ID..."
  entityType="estado"
