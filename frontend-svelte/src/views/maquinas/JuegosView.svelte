...
<script>
  ...
  onMount(async () => {
    ...
    try {
      const local = await getLocalItems('juegos_maquinas');
  ...
  async function loadServerData(params = {}) {
    ...
    try {
      ...
      if (json && json.success) {
        ...
        saveLocalItems('juegos_maquinas', items).catch(() => {});
      }
    } catch (err) {
      console.warn('Fallback local IndexedDB para juegos de máquinas:', err);
      const local = await getLocalItems('juegos_maquinas');
      const source = (Array.isArray(local) && local.length > 0) ? local : ($masterJuegosMaquinasStore || []);
  ...
  async function handleCreate(event) {
    ...
    try {
      const created = await masterJuegosMaquinasActions.add(draft);
  ...
  async function handleSaveInline(event) {
    ...
    const targetUuid = id;
    try {
      await masterJuegosMaquinasActions.update(targetUuid, draft);
  ...
  async function handleDelete(event) {
    ...
    try {
      const res = await masterJuegosMaquinasActions.delete(targetUuid);
      if (res && res.blocked) {
        if (onResult) {
          onResult(res);
        } else {
