<script context="module">
  ...
  export const persistentLegalFilters = writable({
    searchQuery: ""
  });
</script>
...
<script>
  ...
  let initial = {};
  const unsubInit = persistentLegalFilters.subscribe((val) => {
    initial = val || {};
  });
  unsubInit();
  ...
  async function loadServerData(params = {}) {
    ...
    try {
      ...
      const res = await fetch(`/api/master/legal?${q.toString()}`);
      ...
      const local = await getLocalItems('legal', null, 'created_at', 'desc');
...
<PaginatedDataTable 
  ...
  {createFields}
  bind:searchQuery
  searchPlaceholder="Buscar registros legales por nombre o ID..."
  entityType="legal"
