...
<script>
  ...
  onMount(async () => {
    ...
    try {
      const local = await getLocalItems('marcas');
      if (Array.isArray(local) && local.length > 0) {
        items = local;
        totalCount = local.length;
      } else if (Array.isArray(allMarcas) && allMarcas.length > 0) {
        items = allMarcas;
        totalCount = allMarcas.length;
      }
    } catch (e) {}
  ...
  async function loadServerData(params = {}) {
    ...
    try {
      ...
      if (json && json.success) {
        ...
        saveLocalItems('marcas', items).catch(() => {});
      }
    } catch (err) {
      console.warn('Fallback local IndexedDB para marcas:', err);
      const local = await getLocalItems('marcas');
      const source = (Array.isArray(local) && local.length > 0) ? local : ($masterMarcasStore || []);
  ...
  async function handleCreate(event) {
    ...
    try {
      const created = await masterMarcasActions.add(draft);
  ...
  async function handleSaveInline(event) {
    ...
    const targetUuid = id;
    try {
      await masterMarcasActions.update(targetUuid, draft);
  ...
  async function handleDelete(event) {
    ...
    try {
      const res = await masterMarcasActions.delete(targetUuid);
      if (res && res.blocked) {
        if (onResult) {
          onResult(res);
        } else {
          triggerToast(res.message || 'No se puede eliminar porque tiene modelos vinculados.', 'warning');
        }
      } else {
