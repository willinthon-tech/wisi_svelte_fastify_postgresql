<script context="module">
  ...
  export const persistentCargosFilters = writable({
    selectedSalas: [],
...
<script>
  ...
  let initial = {};
  const unsubInit = persistentCargosFilters.subscribe((val) => {
    initial = val || {};
  });
  unsubInit();
  ...
  let lastFilterKey = "";
  $: filterKey = `${(assignedSalaIds || []).join(",")}_${selectedSalas.join(",")}_${selectedDepartamentos.join(",")}_${selectedAreas.join(",")}_${(searchQuery || "").trim()}`;
  ...
  async function fetchFilterOptions() {
    try {
      ...
      const res = await fetch(`/api/master/cargos/filter-options?${q.toString()}`);
      if (res.ok) {
  ...
  async function loadServerData(params = {}) {
    ...
    try {
      ...
      const res = await fetch(`/api/master/cargos?${q.toString()}`);
      const json = await res.json();
  ...
  async function handleBatchDelete(event) {
    ...
    for (const id of ids) {
      try {
        ...
        triggerToast(`Error al eliminar cargo ID ${id}: ${err.message}`, 'error');
      }
    }
    triggerToast(`${ids.length} cargos eliminados en masivo`, 'success');
    onResult({ success: true });
    await loadServerData();
  }
</script>
...
<PaginatedDataTable 
  ...
  {createFields}
  bind:searchQuery
  searchPlaceholder="Buscar cargos por nombre, área o ID..."
  entityType="cargo"
...
>
  <div slot="filters" class="smart-filters-grid">
    ...
    <SmartMultiSelect
      id="filter-cargos-departamentos"
      label="Departamento"
